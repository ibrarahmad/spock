...
int handle_slot_drop_command(int argc, char *argv[])
{
    ...
    optind = 2;
    while ((c = getopt_long(argc, argv, "s:n:h", long_options, &option_index)) != -1) {
        switch (c) {
            case 's':
                slot_name = optarg;
                break;
    ...
    if (!node_name) {
        log_error("Node name is required for drop.");
        print_slot_drop_help();
        return EXIT_FAILURE;
    }
...
int handle_slot_enable_command(int argc, char *argv[])
{
    ...
    if (!node_name) {
        log_error("Node name is required for enable.");
        print_slot_enable_help();
        return EXIT_FAILURE;
    }
    ...
    printf("Enabling replication slot: %s on node: %s (no-op, not implemented)\n", slot_name, node_name);
    return EXIT_SUCCESS;
}
...
int handle_slot_disable_command(int argc, char *argv[])
{
    ...
    if (!node_name) {
        log_error("Node name is required for disable.");
        print_slot_disable_help();
        return EXIT_FAILURE;
    }
    ...
    printf("Disabling replication slot: %s on node: %s (no-op, not implemented)\n", slot_name, node_name);
    return EXIT_SUCCESS;
}
...
void print_slot_drop_help(void)
{
    printf("Usage: spockctrl slot drop --node <node_name> --slot <slot_name>\n");
...
void print_slot_enable_help(void)
{
    printf("Usage: spockctrl slot enable --node <node_name> --slot <slot_name>\n");
...
void print_slot_disable_help(void)
{
    printf("Usage: spockctrl slot disable --node <node_name> --slot <slot_name>\n");
